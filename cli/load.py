    ...
    for line in iter(stdin.readline, b''):
        ...
        if obj is not None:
            ...
            try:
                ...
                reply(act, 'NotFound: ' + nf.message, obj) 
            else:
...
def _upload_resources(ckan,obj,arguments):
    ...
    pck_dict = ckan.action.package_show( **{'id': obj['name']} )
    rs = pck_dict['resources'] 
    
    if len(resources)==0:
        return
    for resource in resources:
        for r in rs:
            if r['name'] == resource['name'] and  r['description'] == resource['description']:
               resource['id'] = r['id']
               break
